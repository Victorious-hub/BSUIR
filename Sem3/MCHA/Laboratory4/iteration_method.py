...
def get_q(fi_equation, approx, e=0.1):
    """
        Проверка условий сходимости системы уравнений.
        Для этого считаем матрицу якоби с частными производными и смотрим проверку на условие сходимости
    """ 
    x_0 = approx[0]
    x_fi = approx[1]
    x = symbols('x:2')
...
def iteration_solve(system_equations: np.array, approx, tol=0.00001, verbose=0):
    ...
    if verbose == 1:
        """
            Если отношение < q(q < 1), сходимость есть и далее считаем приближения до момента, пока отношение не > погрешности
        """
